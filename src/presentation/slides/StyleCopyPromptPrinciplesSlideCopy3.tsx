...
const IMAGES = {
  input: '/images/editing/inpainting.png',
  output: '/images/editing/result2.png',
} as const;

const PROMPT = 'Put the ball in the hand';

export function StyleCopyPromptPrinciplesSlideCopy3({ index, totalSlides }: SlideRenderProps) {
  return (
    <SlideFrame padding="compact">
      ...
      <SlideContent width="wide" density="compact" className="h-full min-h-0 justify-between">
        <SlideHeader
          ...
          meta={
            <Text variant="meta">{formatSlideMeta('Точечное редактирование', index, totalSlides)}</Text>
          }
        />
