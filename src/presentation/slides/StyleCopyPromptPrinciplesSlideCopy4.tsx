...
const IMAGES = {
  input: '/images/editing/var2.png',
  output: '/images/editing/var3.png',
} as const;

const PROMPT = 'Create a new brand character in this style: a tennis player on the court, wearing blue jacket on yellow background';

export function StyleCopyPromptPrinciplesSlideCopy4({ index, totalSlides }: SlideRenderProps) {
  return (
    <SlideFrame padding="compact">
      ...
      <SlideContent width="wide" density="compact" className="h-full min-h-0 justify-between">
        <SlideHeader
          ...
          meta={
            <Text variant="meta">{formatSlideMeta('Копирование стиля', index, totalSlides)}</Text>
          }
        />
        ...
        <SlideGrid columns={12} gap="md" className="min-h-0 flex-1 auto-rows-fr items-stretch">
          <SlideColumn span={3} className="min-h-0">
            ...
            >
              <Text variant="overline">Копирование стиля</Text>
