...
const IMAGES = {
  ref1: '/images/leo/ref1.png',
  ref2: '/images/leo/ref2.jpg',
  ref3: '/images/leo/ref3.jpg',
  result1: '/images/leo/result1.png',
  result2: '/images/leo/result2.png',
} as const;
...
export function LeoWideShotSlide({ index, totalSlides }: SlideRenderProps) {
  return (
    <SlideFrame padding="compact">
      ...
      <SlideContent width="wide" density="compact" className="h-full min-h-0 justify-between">
        <SlideHeader
          ...
          meta={
            <Text variant="meta">{formatSlideMeta('Фотореализм', index, totalSlides)}</Text>
          }
        />
        ...
        <SlideGrid columns={12} gap="md" className="min-h-0 flex-1 auto-rows-fr items-stretch">
          <SlideColumn span={3} className="min-h-0">
            ...
            >
              ...
              <SlidePromptQuote>
                A cat <span className="font-bold">[Image 1]</span> sits on a man&apos;s{' '}
                <span className="font-bold">[Image 2]</span> lap, and <span className="font-bold">[Image 3]</span>{' '}
                sleep on the sofa wide shot
              </SlidePromptQuote>
            </Reveal>
          </SlideColumn>
          ...
          <SlideColumn span={9} className="min-h-0">
            <div
              className="grid h-full min-h-0 w-full min-w-0 grid-cols-[minmax(0,2fr)_minmax(0,3fr)] grid-rows-6 gap-[var(--slide-grid-gap-md)]"
            >
              <Reveal
                preset="soft"
                delay={0.2}
                className="col-start-1 row-span-2 row-start-1 min-h-0 h-full w-full min-w-0"
              >
              ...
              <Reveal
                preset="soft"
                delay={0.24}
                className="col-start-1 row-span-2 row-start-3 min-h-0 h-full w-full min-w-0"
              >
                <div className={`${CELL} h-full`}>
              ...
              <Reveal
                preset="soft"
                delay={0.28}
                className="col-start-1 row-span-2 row-start-5 min-h-0 h-full w-full min-w-0"
              >
                <div className={`${CELL} h-full`}>
                  <img src={IMAGES.ref3} alt="" className={IMG} />
                </div>
              </Reveal>
